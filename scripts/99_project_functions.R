...
## Merging with annotation before DGE ------------------------------------------
# when multiple ps match same ensg, choose ps with highest average exp
...
ps_to_ensg <- function(exp_data, pheno_data, platform){
  mapping <- AnnotationDbi::select(platform,
                                   row.names(exp_data),
                                   c("SYMBOL"),
                                   keytype="PROBEID") %>%
    left_join(rownames_to_column(as.data.frame(exp_data), var="PROBEID")) %>%
    dplyr::rename(Gene.name = "SYMBOL") %>%
    left_join(dplyr::select(Final_Annotation_List, Gene.name, ENSG.ID),
              relationship = "many-to-many") %>% 
    drop_na()  # now I need to calculate average exp
  
  mapping <- mapping %>%
    mutate(ave_exp = apply(mapping[, pheno_data$geo_accession], 1, mean)) %>%
    group_by(ENSG.ID) %>%
    arrange(desc(ave_exp), .by_group=TRUE) %>% # cases where one ensg matches multiple probes take the one with hightest ave_exp
    filter(row_number()==1) %>%
    ungroup()
  ...
  dups_gene.name <- mapping %>% 
    count(Gene.name) %>%
    filter(n>1)
  
  mapping <- mapping %>%
    filter(!(PROBEID %in% dups_ps$PROBEID)) %>%
    filter(!(Gene.name %in% dups_gene.name$Gene.name)) %>%
    column_to_rownames("ENSG.ID") %>%
    dplyr::select(-c("Gene.name", "PROBEID", "ave_exp"))
...
#}
#
## probe sets to ENSG ids for hgug4112a annotation -----------------------------
library(hgug4112a.db)
library(illuminaHumanv4.db)
...
AnnotationDbi_mapping <- function(data, platform){
  mapping <- AnnotationDbi::select(platform,
                                   row.names(data),
                                   c("SYMBOL"),
                                   keytype="PROBEID") %>%
    left_join(rownames_to_column(data, var="PROBEID")) %>%
    dplyr::rename(Gene.name = "SYMBOL") %>%
    left_join(Final_Annotation_List, relationship = "many-to-many") %>% 
    drop_na() %>%
    group_by(ENSG.ID) %>%
    arrange(desc(abs(logFC)), .by_group=TRUE) %>% # cases where one ensg matches multiple probes take the one with hightest LFC
    filter(row_number()==1) %>%
  ...
  dups <- mapping %>% 
    count(Gene.name) %>%
    filter(n>1) # these duplicates come from final annotation, not from probesets
  
  mapping <- mapping %>%
    filter(!(Gene.name %in% dups$Gene.name))
  
  return(mapping)
}


## Merge DGE results with gene metadata and optionally filter protein coding ---
merge_genes_metadata <- function(DGE_res, gene_metadat, only_coding){
  DGE_res <- DGE_res %>%
    rownames_to_column("ENSG.ID") %>%
    left_join(gene_metadat)
  
  if(only_coding == TRUE){DGE_res <- filter(DGE_res, Gene.type == "protein_coding")}
...
## HGU133Plus2 mapping for correlation approach --------------------------------

# in the DEG approach I was chosing ps with highest LFC
# should I chose highest correlation? highest average expression? 
HGU133Plus2_map_corr <- function(genes_cor_df){
  genes_cor_df <- genes_cor_df %>%
    left_join(distinct(group_by(HGU133Plus2, ENSG.ID, ps))) %>% 
    drop_na() %>%
    group_by(ENSG.ID) %>%
    arrange(desc(av_exp), .by_group=TRUE) %>% # cases where one ensg matches multiple probes take the one with hightest average exp
    filter(row_number()==1) %>%
    ungroup() %>%
    group_by(ps) %>%
    mutate(ps_count = n()) %>%
